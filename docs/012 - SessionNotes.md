...
## Session: Bug Fixes — Card Overflow, Book Now, All Filter, Checkmark, Admin Images

**Date:** 2026-03-07

### Bug 1 — Service Card Overflow (1px bottom overflow)

**Root cause:** `_ServiceCard` used an unbounded `SizedBox(width:162)` with a `Column` that had no height constraint. The "View details" row + padding caused the column to exceed the parent `SizedBox(height:232)` list container by 1px.
**Fix:**

- Gave `_ServiceCard.build` an explicit `Container(width:162, height:220)`
- Wrapped the inner `Column` in `ClipRRect(borderRadius:18)` to clip any residual overflow
- Removed the "View details" row entirely — redundant since the whole card is tappable
- Used `Expanded` for the info section so it fills remaining height without overflowing
- Changed `Stack(fit:StackFit.expand)` for the image so it fills the fixed 120px exactly

### Bug 2 — Book Now goes back to Dashboard instead of Request Service

**Root cause:** `PopScope(canPop: false)` in `main.dart._buildWrapper()` intercepts ALL back gestures, including the `Navigator.pop(type)` call from `ServiceDetailScreen.onBookNow`. The interceptor fired `_screen = 'home'` before the awaited result returned to `_openServiceDetail`. The `setState(_screen='request_service')` then ran but was overwritten.
**Fix (main.dart):**
...
onPopInvokedWithResult: (didPop, _) {
  ...
  if (nav.canPop()) {   // ← route on top (e.g. ServiceDetailScreen)
    nav.pop();           // ← let it pop normally, don't intercept
    return;
  }
  // ... rest of back-button logic
}
```

This lets `Navigator.pop(type)` complete normally → `_openServiceDetail` receives the `String` result → calls `onRequestServiceWithType(type)` → `setState(_screen='request_service')` → Request Service screen opens with type pre-filled.

### Bug 3 — "All" filter doesn't restore original professional count

**Root cause:** `onFilterBySkill` in `main.dart` only called `getProfessionals(skill:skill)`. Selecting "All" didn't call `onFilterBySkill` at all (guarded by `if (cat.label != 'All')`), so `_professionals` stayed as the last filtered subset.
**Fix (two parts):**

1. `dashboard_customer.dart`: Removed `if (cat.label != 'All')` guard — now ALL chip selections call `onFilterBySkill`, including "All"
2. `main.dart`: Added branch for 'All' → calls `getProfessionals()` (no skill filter) to reload the full list:
...
### Bug 4 — Checkmark icon on selected FilterChip is redundant

**Fix:** Added `showCheckmark: false` to every `FilterChip` in the Service Offers category row. The selected state is already visually clear from the filled background color.

### Bug 5 — Admin document images don't load (show broken icon)

**Root cause:** Supabase Storage buckets (`credentials`, `valid_ids`) are private by default. `Image.network` has no way to attach auth headers, so requests return 401/403 and trigger `errorBuilder`.
**Immediate Flutter fix (approvals_admin.dart):**

- `_docTile`: Added `loadingBuilder` with a small `CircularProgressIndicator` while the image loads; improved error state shows a cleaner icon instead of `image_not_supported`
- `_showDocDialog`: Completely reworked — dark background dialog, `InteractiveViewer` (pinch-to-zoom up to 5×), proper `loadingBuilder` with progress indicator showing % loaded, and an error state that explains the likely cause (private bucket)

**Required Supabase action (must do in dashboard):**

1. Go to Supabase Dashboard → Storage → `credentials` bucket → Policies
2. Add a policy: `SELECT` for `authenticated` role (or set bucket to Public if images are non-sensitive)
3. Repeat for `valid_ids` bucket
   Without this, images will continue to fail regardless of Flutter-side changes.
