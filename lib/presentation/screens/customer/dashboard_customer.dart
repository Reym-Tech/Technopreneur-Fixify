...
class CustomerDashboardScreen extends StatefulWidget {
  ...
  /// AYO Elite handymen (Tier 2) — shown in the Featured row above the service
  /// catalogue. Filtered from the full professionals list in main.dart so only
  /// active, available Elite handymen appear here.
  final List<ProfessionalEntity> featuredProfessionals;
  ...
  const CustomerDashboardScreen({
    ...
    this.featuredProfessionals = const [],
  });
...
class _CustomerDashboardScreenState extends State<CustomerDashboardScreen> {
  ...
  // ── FEATURED HANDYMEN ROW (AYO Elite) ────────────────────────────────────
  // Horizontal scroll row showing Tier 2 handymen. Only rendered when
  // featuredProfessionals is non-empty — see build() conditional above.
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemCount: widget.featuredProfessionals.length,
            separatorBuilder: (_, __) => const SizedBox(width: 12),
            itemBuilder: (ctx, i) {
              final pro = widget.featuredProfessionals[i];
              return _FeaturedProCard(
                professional: pro,
                onTap: widget.onProfessionalTap != null
                    ? () => widget.onProfessionalTap!(pro)
                    : null,
              );
            },
          ),
...
// ── Featured Pro Card ─────────────────────────────────────────────────────────
// Compact horizontal card used in the Featured Handymen row on the customer
// dashboard. Only shown for AYO Elite (Tier 2) handymen.

class _FeaturedProCard extends StatelessWidget {
  final ProfessionalEntity professional;
  final VoidCallback? onTap;

  const _FeaturedProCard({required this.professional, this.onTap});

  @override
  Widget build(BuildContext context) {
    final hasAvatar = professional.avatarUrl?.isNotEmpty == true;
    final initials = professional.name
        .trim()
        .split(' ')
        .take(2)
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(
                    color: const Color(0xFFFF9500).withOpacity(0.4), width: 2),
              ),
              child: ClipOval(
                child: hasAvatar
